...
async def async_setup_entry(
    ...
    if config.get(CONF_SWITCH_ENTITY_ID):
        sensors.extend([
...
class BaseConfigSensor(SensorEntity):
    ...
    def __init__(self, hass: HomeAssistant, entry: ConfigEntry) -> None:
        ...
        self.hass = hass
        self._entry = entry
        # Note: Sensors cannot have EntityCategory.CONFIG - that's only for diagnostic entities
...
class CastDelaySensor(BaseConfigSensor):
    ...
    def _update_from_config(self, entry: ConfigEntry) -> None:
        """Update the sensor value from config entry."""
        # Check options first, then data as fallback
        config = dict(entry.data)
        config.update(entry.options)
        self._attr_native_value = config.get(CONF_CAST_DELAY, DEFAULT_CAST_DELAY)
...
class LoggingLevelSensor(BaseConfigSensor):
    ...
    def _update_from_config(self, entry: ConfigEntry) -> None:
        ...
        config = dict(entry.data)
        config.update(entry.options)
        self._attr_native_value = config.get(CONF_LOGGING_LEVEL, DEFAULT_LOGGING_LEVEL)
...
class StartTimeSensor(BaseConfigSensor):
    ...
    def _update_from_config(self, entry: ConfigEntry) -> None:
        ...
        config = dict(entry.data)
        config.update(entry.options)
        self._attr_native_value = config.get(CONF_START_TIME, DEFAULT_START_TIME)
...
class EndTimeSensor(BaseConfigSensor):
    ...
    def _update_from_config(self, entry: ConfigEntry) -> None:
        ...
        config = dict(entry.data)
        config.update(entry.options)
        self._attr_native_value = config.get(CONF_END_TIME, DEFAULT_END_TIME)
...
class SwitchEntitySensor(BaseConfigSensor):
    ...
    def _update_from_config(self, entry: ConfigEntry) -> None:
        ...
        config = dict(entry.data)
        config.update(entry.options)
        self._attr_native_value = config.get(CONF_SWITCH_ENTITY_ID, "")
...
class SwitchStateSensor(BaseConfigSensor):
    ...
    def _update_from_config(self, entry: ConfigEntry) -> None:
        ...
        config = dict(entry.data)
        config.update(entry.options)
        self._attr_native_value = config.get(CONF_SWITCH_ENTITY_STATE, "")
